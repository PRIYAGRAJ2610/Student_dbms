...
class Student:
    ...
    def __init__(self,root):
       ...
       self.root.geometry(newGeometry="1328x650+0+0")
       ...
       pb.studentData()
       def iExit():
              iExit=tkinter.messagebox.askyesno("priyag's student dbms","conform if you want to exit")
              if iExit>0:
                     root.destroy()
                     return
       def clearData():
       ...
       TitFrame=Frame(MainFrame,bd=1,padx=54,pady=8,bg="black",relief=RIDGE)
       TitFrame.pack(side=TOP)#THIS IS STITLE FRAME
    
       self.lblTit=Label(TitFrame,font=('arial',47,'bold'),text="Students Database Management System",bg="black",fg="orange")
       self.lblTit.grid()

       self.lblTit=Label(TitFrame,font=('arial',40,'bold'),text="RKGIT , Ghaziabad",bg="black",fg="orange")
       self.lblTit.grid()

       self.lblTit=Label(TitFrame,font=('arial',18),text="(created by- PRIYAG RAJ SHARMA , SHALINI , PRIYA )",bg="black",fg="white")
       self.lblTit.grid()
